...
const EditGallery = () => {
      ...
      if (res.data?.success) {
        ...
        setFormData({
          ...
          images: gallery.images.map(img => ({
            ...img,
            id: img._id || Math.random(), // Existing images have _id from backend
          }))
        });
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await withLoading(() => api.put(`/gallery/${id}`, {
        ...formData,
        images: imagesPayload
      }), t('common.saving'));

      if (res.data?.success) {
        addToast(t('gallery.admin.updatedSuccess'), 'success');
        navigate('/admin/gallery');
      } else {
  ...
  return (
    <AdminPageLayout
      ...
      title={t('gallery.admin.editGallery')}
      subtitle={formData.title}
      actions={
        <button className="btn btn-secondary" onClick={() => navigate('/admin/gallery')}>
          <ArrowLeft size={18} />
          {t('common.back')}
        </button>
      }
              ...
              <div className="u-flex u-flex-wrap u-gap-md u-mt-lg">
                {formData.images.map((img) => (
                  <div key={img.id} className="admin-form-image-preview" style={{ width: '180px', height: '120px' }}>
                    <img src={img.url} alt={img.name || img.alt} />
